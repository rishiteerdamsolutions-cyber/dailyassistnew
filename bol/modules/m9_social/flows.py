...
FACEBOOK_POST = SocialFlow(
    ...
    steps=[
        ...
        Step(number=5,
             ...
             action=ActionType.CLICK,
             template="facebook_s5_photo_icon",
             text_fallback="Photo",
             optional_if_no_key="media_path",
             wait_seconds=2.0,
             notes="Green photo icon at bottom of composer toolbar. OCR fallback uses 'Photo' (without slash — more reliable). Opens the photo upload area."),
        Step(number=6,
        ...
        Step(number=8,
             ...
             text_fallback="B-Post",
             wait_seconds=5.0,
             notes="FINAL STEP: Pure OCR: finds 'Post' as a button shape. We DO NOT use an anchor because Facebook hides toolbar labels (like 'Feeling') after a photo is uploaded. We DO NOT use a template because the visual button is just a solid blue rectangle that falsely matches other blue UI elements."),
        Step(number=9, description="Post published successfully", action=ActionType.CONFIRM_DONE),
    ]
)
...
LINKEDIN_POST = SocialFlow(
    ...
    steps=[
        ...
        Step(number=5,
             ...
             optional_if_no_key="media_path",
             wait_seconds=2.5,
             notes="After selecting a file, LinkedIn shows a preview editor. You MUST click 'Next' here to return to the composer. Only do this if media was uploaded."),
        Step(number=6,
...
X_POST = SocialFlow(
    ...
    steps=[
        ...
        Step(number=6,
             ...
             text_fallback="B-Post",
             spatial_anchor_right_of="Everyone",
             wait_seconds=3.0,
             notes="FINAL STEP: Black Post button inside the composer. Anchored right_of='Everyone' so it never clicks a feed post."),
