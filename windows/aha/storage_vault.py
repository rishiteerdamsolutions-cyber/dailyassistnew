...
def calendar_total_days_env() -> int:
    raw = (os.environ.get("AHA_CALENDAR_TOTAL_DAYS") or "30").strip()
    try:
        n = int(raw)
    except ValueError:
        n = 30
...
def ensure_vault_root(downloads_base: Path | None = None) -> Path:
    """Create ``Downloads/aha`` only (no plan subfolders)."""
    root = vault_root(downloads_base)
...
def ensure_plan_vault(
    ...
    *,
    ...
    stubs = 0
    skip_touch = (os.environ.get("AHA_SKIP_30DAY_TOUCH") or "").strip().lower() in ("1", "true", "yes")
...
def ensure_slot_stubs(downloads_base: Path | None = None) -> int:
    if (os.environ.get("AHA_SKIP_30DAY_TOUCH") or "").strip().lower() in ("1", "true", "yes"):
        return 0
    total = calendar_total_days_env()
    n = 0
    for day in range(1, total + 1):
...
def bootstrap_storage_vault(
    ...
    *,
    ...
    """
    ``minimal`` (default): only ``Downloads/aha`` — plan folders are created when
    ...
    if (os.environ.get("AHA_STORAGE_WRITE_DOCS") or "").strip().lower() in ("1", "true", "yes"):
...
def list_aha_legacy_top_level(downloads_base: Path | None = None) -> dict[str, Any]:
    """
    Names under ``Downloads/aha`` that are **not** part of the current plan vault
...
def cleanup_aha_legacy_top_level(
    downloads_base: Path | None = None,
    *,
