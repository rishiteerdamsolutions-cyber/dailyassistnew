...
class SocialFlowExecutor:
    ...
    def _execute_step(self, step: Step, params: dict, flow: SocialFlow) -> bool:
        ...
        if step.action == ActionType.NAVIGATE:
            ...
            # 1. Navigate and wait for DOM load
            applescript = f'''
            tell application "Google Chrome"
                activate
                open location "{url}"
            ...
            # 2. Maximize the window ONLY AFTER it's completely loaded (bypassing permissions)
            try:
                try:
                    ...
                    logger.warning("pygetwindow not installed. Please pip install pygetwindow for robust Windows focusing.")
                
                time.sleep(0.5)
                # F11 is native Windows Chrome fullscreen
                pyautogui.press('f11')
                time.sleep(1.5) # Wait for the fullscreen animation to finish
            except Exception as e:
                logger.error("Failed to maximize Chrome automatically on Windows: %s", e)
        ...
        if step.action == ActionType.OS_OPEN:
            ...
            if media_path:
                try:
                    ...
                    try:
                        import pyperclip
                        pyperclip.copy(str(media_path))
                        logger.info(f"OS_OPEN: pyperclip copy successful, pasting now")
            
                        time.sleep(1.0) # Ensure Windows File Dialog has loaded and focused the 'File name' box
            
                        # Windows file dialog automatically focuses the File Name box
                        # Just paste the path directly
                        pyautogui.hotkey('ctrl', 'v')
                        time.sleep(1.0)
            
                        # Hit enter to submit the path/file
                        pyautogui.press('enter')
            
                        logger.info("OS_OPEN: Windows file upload flow COMPLETED. Returning True.")
                        return True      
                    except ImportError:
                        logger.warning("OS_OPEN: pyperclip missing! Falling back to typewrite.")
                        pyautogui.typewrite(str(media_path), interval=0.03)
                        time.sleep(1.0)
                        
                        # Hit enter to submit the path/file
                        pyautogui.press('enter')
                        logger.info("OS_OPEN: Windows file upload flow (typewrite fallback) COMPLETED. Returning True.")
                        return True
                except Exception as e:
                    with open("/Users/nandagiriaditya/GEMINI/ARTIFICIALHUMANAGENT/executor_debug.log", "a") as f:
                        f.write(f"OS_OPEN: CRASHED: {str(e)}\n")
                    raise

            # No media_path — fall back to visually finding and clicking the Open button
            logger.info("OS_OPEN: media_path was EMPTY! Falling back to visual click.")
            try:
                with open("/Users/nandagiriaditya/GEMINI/ARTIFICIALHUMANAGENT/executor_debug.log", "a") as f:
                    f.write("OS_OPEN: media_path EMPTY — falling back to visual click.\n")
            except Exception:
                pass
            self._verify_stable_state(step)
            return self._wait_and_find_and_click(step, max_wait=max(15.0, step.wait_seconds))
        ...
        if step.action == ActionType.UPLOAD:
            ...
            if media_path:
                ...
                time.sleep(0.5)
                pyautogui.press('return')
                time.sleep(1.5)  # INCREASED: wait for Go to Folder sheet to close completely
                pyautogui.press('return')  # Confirm selection
                time.sleep(1.5)  # Wait for dialog to close and browser to process
            return True
