...
def _send_ghost_packet(payload: dict) -> None:
    import json
    import socket
    try:
...
def _find_and_click(
    ...
    """
    Try template match first, then OCR text fallback using semantic/spatial logic.
    Returns True if something was clicked.
    """
    import cv2
    _send_ghost_packet({"type": "clear"}) # Clear old visuals
    ...
    if step.template:
        ...
        if match:
            ...
            _send_ghost_packet({"type": "laser", "x": cx, "y": cy, "color": "green"})
    ...
    # 2. OCR text fallback (using advanced OCREngine for semantics & spatial logic)
    if step.text_fallback:
        ...
        # Filter out URL bar hits (top 120 pixels of Chrome window)
        bboxes = [b for b in bboxes if b.y > 120]
        
        if bboxes:
            box = bboxes[0] # Pick the first/best match
            
            # Send all other matches to Ghost Overlay as Visual Guard Threats
            if len(bboxes) > 1:
                for bad_box in bboxes[1:]:
                    bx = bad_box.x + chrome_offset[0]
                    by = bad_box.y + chrome_offset[1]
                    _send_ghost_packet({"type": "threat", "bbox": [bx, by, bad_box.width, bad_box.height]})
            ...
            _send_ghost_packet({"type": "laser", "x": x, "y": y, "color": "blue"})
            _human_click(x, y, ignore_bounds=(step.action == ActionType.OS_OPEN))
            logger.info("OCR Semantic match '%s' → clicked (%d, %d)", step.text_fallback, x, y)
            return True
...
class SocialFlowExecutor:
    ...
    def _dismiss_obstructive_modals(self, step: Step) -> bool:
        ...
        dismiss_words = ["Not Now", "Cancel", "Close", "No Thanks", "Leave", "Skip", "Decline", "Dismiss"]
    ...
    def _wait_and_find_and_click(self, step: Step, max_wait: float = 15.0) -> bool:
        ...
        while time.time() - start_time < max_wait:
            ...
            # Intelligent Auto-Scrolling: If halfway through timeout, try scrolling down
            if elapsed > (max_wait / 2.0) and not has_scrolled and step.action != ActionType.OS_OPEN:
                logger.info("Halfway through timeout, attempting to scroll down to find element...")
