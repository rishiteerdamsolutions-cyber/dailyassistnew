"""
Storage vault layout under ``Downloads/aha/`` (global, plan-based).
...
slot media, but all concrete paths are defined in ``aha.storage_vault``.
"""
...
def ensure_aha_platform_tree(base_dir: Path | None = None) -> dict[str, dict[str, Path]]:
    """
    Paths used by platform-specific AI export.

    Returns: platform_key → {"text","image","video"} → Path under ``Downloads/aha/AI Pro/<Platform>/…``.
    """
...
def bootstrap_aha_content_folders(
    *,
    workflows_dir: Path,
    bundles_dir: Path,
    ...
    The prior implementation created per-workflow trees under ``Downloads/aha/<flow>/<workflow>/``.
    The new system creates one global vault under ``Downloads/aha/``.
    """
...
def calendar_cycles_wrap_env() -> bool:
    raw = (os.environ.get("AHA_CALENDAR_CYCLE") or "wrap").strip().lower()
...
def write_calendar_slot_media(
    ...
    *,
    ...
    """
    Write one file under ``…/Downloads/aha/<flow>/<workflow>/{Core|Hybrid|AI}/``
