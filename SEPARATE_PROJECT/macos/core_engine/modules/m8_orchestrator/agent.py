...
class AutonomousCompanion:
    ...
    def step(self, user_message: str | None = None, is_native_app: bool = False):
            ...
            if flow:
                ...
                if slot_base.exists() and "media_path" not in params:
                    ...
                    if matched_slot_dir:
                        ...
                        found_img = None
                        if img_dir.exists():
                            # 1. Try today's exact date match
                            for ext in [".png", ".jpg", ".jpeg", ".webp", ".gif"]:
                                p = img_dir / f"{today_day}{ext}"
                                if p.exists() and p.stat().st_size > 0:
                                    found_img = p
                                    break
                            # 2. Blind fallback: Grab ANY image in the vault for this platform
                            if not found_img:
                        ...
                        if vid_dir.exists():
                            ...
                            for ext in [".mp4", ".mov", ".webm"]:
                                ...
                                if p.exists() and p.stat().st_size > 0:
                                    found_vid = p
                                    break
                            # 2. Blind fallback: Grab ANY video in the vault for this platform
                            if not found_vid:
                        ...
                        # DEBUG LOG DUMP
                        try:
                            with open("/Users/nandagiriaditya/GEMINI/ARTIFICIALHUMANAGENT/vault_debug.log", "a") as f:
                                f.write(f"\n--- Vault Debug ---\n")
                                f.write(f"wants_text_only: {wants_text_only}\n")
                                f.write(f"wants_image: {wants_image}, wants_video: {wants_video}\n")
                                f.write(f"found_img: {found_img}\n")
                                f.write(f"params['media_path']: {params.get('media_path')}\n")
                        except Exception:
                            pass
                ...
                # ── Flow Upgrade Check ───────────────────────────────────────
                # If the parsed flow is text-only but we pulled media from the Vault,
                # we must upgrade the flow to handle media uploading.
                ...
                if flow.platform == "instagram" and not params.get("media_path"):
                    msg = "❌ I cannot post to Instagram because no image or video was found! Instagram requires media. Please attach a file or schedule one in the Vault."
                    logger.error(msg)
        ...
        # Determine routing to OpenAI vs Gemini
        use_openai = False
        if use_pro and self.config.openai_api_key:
            use_openai = True
            model_name = self.config.openai_model_name
        else:
            model_name = "gemini-1.5-pro" if use_pro else self.config.gemini_model_name
        ...
        try:
            ...
            if action == "open_url":
                ...
                if is_native_app:
                    ...
                    messages_out.append(f"Opening URL strictly in Google Chrome: {url}")
                    
                    import subprocess, sys
                    try:
                        # STRICT RULE: ONLY OPEN IN GOOGLE CHROME.
                        # AppleScript will force Chrome to open/activate, bypassing the default browser.
                        applescript = f'''
                        ...
                        # Get Chrome window bounds via AppleScript and update browser region
                        bounds_script = '''
                        tell application "Google Chrome"
                        ...
                        '''
                        result = subprocess.check_output(['osascript', '-e', bounds_script]).decode().strip()
                        parts = [int(x.strip()) for x in result.split(',')]
                        x1, y1, x2, y2 = parts
                        # Update the global browser region so captures target Chrome
                        import requests as _req
                        try:
                            _req.post("http://127.0.0.1:8000/api/config/set_browser_region", json={
                                "x": x1, "y": y1, "width": x2 - x1, "height": y2 - y1
                            }, timeout=1)
                        except Exception:
                            pass
                        messages_out.append(f"Chrome opened at region ({x1},{y1},{x2-x1}×{y2-y1}) — agent now watching Chrome.")
                    except Exception as e:
                        logger.warning(f"AppleScript Chrome open failed: {e}. Falling back to 'open -a Google Chrome'.")
                        # Fallback: strictly Google Chrome, no webbrowser module default.
