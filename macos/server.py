...
def agent_chat(req: AgentChatRequest):
    from aha.release_bridge import post_agent_chat, pre_agent_chat

    try:
        block, flow = pre_agent_chat(req.text)
        if block:
            return block
        agent = get_agent()
        result = agent.step(user_message=req.text, is_native_app=req.is_native_app)
        return post_agent_chat(req.text, result, flow)
    except Exception as e:
...
def agent_resume(req: AgentChatRequest):
    try:
        agent = get_agent()
        result = agent.step(user_message="User has completed the manual step. Please resume workflow.", is_native_app=req.is_native_app)
        return result
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def generate_vault_texts(req: GenerateVaultBatchRequest):
    """BYOK only — N captions into consecutive vault days for a slot."""
    ...
    return {
        "status": "success",
        "message": (
            f"Generating {req.num_days} captions for '{slot}'. "
            f"Refresh the calendar in a few seconds."
        ),
    }
