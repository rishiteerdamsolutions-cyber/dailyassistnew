...
def test_bootstrap_storage_vault_creates_expected_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AHA_CALENDAR_TOTAL_DAYS", "30")
    monkeypatch.delenv("AHA_SKIP_30DAY_TOUCH", raising=False)

    from aha.storage_vault import bootstrap_storage_vault, slot_path, vault_root
...
def test_minimal_bootstrap_then_ensure_hybrid(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from aha.storage_vault import bootstrap_storage_vault, ensure_plan_vault, slot_path, vault_root
...
def test_cleanup_legacy_top_level(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from aha.storage_vault import (
        ...
        cleanup_aha_legacy_top_level,
        list_aha_legacy_top_level,
        vault_root,
    )
